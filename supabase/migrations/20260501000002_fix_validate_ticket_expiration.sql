  ...
  IF p_scanner_id IS NOT NULL THEN
    ...
    IF v_scanner.allowed_ticket_types IS NOT NULL AND array_length(v_scanner.allowed_ticket_types, 1) > 0 THEN
      ...
      LOOP
        ...
        -- If scanner allows 'GENERAL', also allow 'NORMAL' type tickets
        IF upper(trim(v_allowed_type)) = 'GENERAL' AND v_ticket_category_to_check = 'NORMAL' THEN
          v_is_allowed := true;
          EXIT;
        END IF;

        -- If scanner allows 'GENERAL', also allow RRPP_FREE but ONLY if the RRPP zone is general (not VIP)
        IF upper(trim(v_allowed_type)) = 'GENERAL' AND v_ticket_category_to_check = 'GENERAL' THEN
          v_is_allowed := true;
          EXIT;
        END IF;
