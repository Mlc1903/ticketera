-- Fix scanner validation for RRPP tickets by zone
...
BEGIN
  ...
  IF p_scanner_id IS NOT NULL THEN
    ...
    -- If it's a mesa (table), we must check the zone category!
    IF v_reservation.res_type = 'mesa_vip' THEN
      IF v_reservation.zone_table_id IS NOT NULL THEN
        ...
        END IF;
      END IF;
    -- If it's an RRPP Free ticket, check the RRPP's zone assignment
    ELSIF (v_reservation.res_type = 'rrpp_free' OR v_ticket_type.type = 'rrpp_free') AND v_reservation.rrpp_id IS NOT NULL THEN
      BEGIN
        SELECT zone_type::text INTO v_rrpp_zone
        FROM public.rrpp_assignments
        WHERE user_id = v_reservation.rrpp_id AND event_id = v_reservation.event_id;
    ...
    IF v_scanner.allowed_ticket_types IS NOT NULL AND array_length(v_scanner.allowed_ticket_types, 1) > 0 THEN
      ...
      LOOP
        -- Fuzzy match: check if the effective category OR the literal ticket name match the allowed scanner categories
        ...
        -- Special case: If scanner allows 'GENERAL', also allow 'NORMAL' and 'RRPP_FREE'
        -- BUT ONLY IF v_ticket_category_to_check hasn't been strictly overridden to VIP
