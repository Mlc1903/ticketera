-- Migration to change organization_zones.category type to TEXT and update validate_ticket to be fully dynamic
-- Path: d:\event-sphere-main\supabase\migrations\20260605000000_update_zone_categories.sql

-- 1. Alter category column in organization_zones
ALTER TABLE public.organization_zones ALTER COLUMN category TYPE TEXT USING category::text;

-- 2. Re-create validate_ticket function with support for dynamic custom categories
...
BEGIN
  ...
  IF p_scanner_id IS NOT NULL THEN
    ...
    IF v_reservation.res_type = 'mesa_vip' THEN
      ...
      IF v_zone_category IS NOT NULL THEN
        v_ticket_category_to_check := upper(trim(v_zone_category));
        v_display_ticket_name := 'MESA - ' || v_ticket_category_to_check;
      ELSE
    ...
    ELSIF v_reservation.rrpp_id IS NOT NULL THEN
      BEGIN
        ...
        IF v_rrpp_zone IS NOT NULL THEN
          v_ticket_category_to_check := upper(trim(v_rrpp_zone));
          IF v_reservation.res_type = 'rrpp_free' THEN v_display_ticket_name := 'Pase ' || v_ticket_category_to_check || ' (RRPP)'; END IF;
        END IF;
      EXCEPTION WHEN OTHERS THEN
        -- fallback silently
      END;
  ...
  IF v_event.is_free_pass AND v_is_free_ticket AND v_event.free_pass_until IS NOT NULL THEN
    -- Handle times past midnight (e.g., 01:30:00) by adding 1 day to the event date
    IF v_event.free_pass_until < '12:00:00'::time THEN
      v_expiration := v_event.date + interval '1 day' + v_event.free_pass_until;
    ELSE
      v_expiration := v_event.date + v_event.free_pass_until;
    END IF;
  ELSE
    -- All other tickets (including admin generated) are valid until 4:00 AM the next day
    v_expiration := v_event.date + interval '1 day' + interval '4 hours';
  END IF;
